...
def fmt_rev(repo, rev):
    ...
    try:
        if is_sha(rev):
            refs = [f'`{r.name}`' for r in (repo.get_branches() +
                                            repo.get_tags()) if rev ==
                                                                b.commit.sha]
...
def _get_merge_status(len_a, len_r, len_pr, len_meta, impostor_shas,
    ...
    if invalid_revs:
        strs.append(f'{invalid_revs} inexistent revision{plural(impostor_shas)}')
    if unreachables:
...
def main():
    ...
    for p in sorted(projs, key=lambda _p: _p[0]):
        ...
        if p in pr_projs:
            ...
            (valid_rev, impostor) = is_valid_rev(repo, rev, check_impostor)
            if impostor:
                impostor_shas += 1
                line += f'|\u274c Impostor SHA: {fmt_rev(repo, new_rev)}{nr_note} '
            elif not valid_rev:
                invalid_revs += 1
                line += f'|\u274c Inexistent revision: {fmt_rev(repo, new_rev)}{nr_note}'
            else:
